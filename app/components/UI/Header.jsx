      ...
      <header>
        <div className="bg-lightBeige w-full flex items-center justify-between px-4 pt-5 pb-4 relative z-20 lg:hidden">
          <Link href="/" className="flex flex-col items-center gap-1">
            <BolerLogo />
            <span className="font-medium uppercase tracking-wider">Bøler Synssenter</span>
          </Link>
          ...
          >
            <span className="text-sm uppercase">{openMenu ? "Lukk" : "Meny"}</span>
            <div className="w-7 h-5 relative">
              <span className={` w-full h-0.5 absolute transition-all ease-in-out duration-300 top-0 ${openMenu && "rotate-45 translate-y-2"}`}></span>
              <span className={`w-full h-0.5 absolute transition-all ease-in-out duration-300 top-2 ${openMenu && "opacity-0"}`}></span>
              <span className={`w-full h-0.5 absolute transition-all ease-in-out duration-300 top-4 ${openMenu && "-rotate-45 -translate-y-2"}`}></span>
            </div>
          </div>
        </div>

        <nav className={`bg-lightBeige w-full h-screen absolute z-10 lg:hidden ${openMenu ? "visible" : "hidden"}`}>
        ...
        <div className="w-full max-w-[1460px] flex flex-col items-center justify-between mx-auto px-10 py-4">
          ...
          <div className="w-full flex items-end justify-between">
            <Link href="/" className="flex flex-col gap-1 items-center">
              <BolerLogo />
              <span className="font-medium uppercase tracking-wider hidden lg:inline">Bøler Synssenter</span>
            </Link>
