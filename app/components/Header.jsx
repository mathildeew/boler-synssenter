...
export default function Header() {
  ...
  return (
    <>
        ...
        <div className="bg-white w-full h-20 px-4 flex items-center justify-between lg:z-auto lg:w-full lg:h-32 lg:px-10">
          ...
          >
            ...
            <div className="w-7 h-7 relative">
              <span
                className={`hamburger top-1 ${
                  openMenu && "rotate-45 translate-y-2"
                }`}
              ></span>
              <span
                className={`hamburger top-3 ${openMenu && "opacity-0"}`}
              ></span>
              <span
                className={`hamburger top-5 ${
                  openMenu && "-rotate-45 -translate-y-2"
                }`}
              ></span>
            </div>
          ...
          <nav
            className={`bg-white translate-all ease-in-out duration-700 z-10 hidden lg:inline`}
          >
            <div className="flex flex-row gap-10 items-center h-full">
              <Link
                href="/synsundersokelse"
              ...
              <a
                className="bg-lightPurple rounded-full px-8 py-2 w-fit"
                data-animate-in="true"
      ...
      <nav
        className={`bg-white w-full absolute translate-all ease-in-out duration-700 z-10 lg:inline lg:translate-y-0  lg:relative lg:w-auto ${
          openMenu ? "top-20 h-screen" : "-top-[400px] h-20 lg:h-auto"
        }`}
      >
        <div className="flex flex-col items-end gap-12 px-4 py-12 lg:hidden">
          ...
          <a className="bg-lightPurple rounded-full px-6 py-2 w-fit">
            Bestill kontaktlinser
          </a>
        </div>
      </nav>
